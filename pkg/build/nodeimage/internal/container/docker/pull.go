...
func PullIfNotPresent(logger log.Logger, image, arch string, retries int) (pulled bool, err error) {
	...
	return true, Pull(logger, image, arch, retries)
}

// Pull pulls an image, retrying up to retries times
func Pull(logger log.Logger, image, arch string, retries int) error {
	logger.V(1).Infof("Pulling image: %s ...", image)
	err := exec.Command("docker", "pull", "--platform", arch, image).Run()
